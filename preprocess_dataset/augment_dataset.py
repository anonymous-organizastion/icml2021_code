...
"""
#Datasets: 2500_shelf_stacking, 2500_shelf_4x4_balanced_nd
def main(args):
    print('Args: '+str(args))
    parser = argparse.ArgumentParser()
    parser.add_argument('--pkl_dataset', type=str, required=False, default='datasets/2500_shelf_stacking_all_distractors',
                        help='dataset')
    parser.add_argument('--noac_swaps', type=int, required=False, default=0,
                        help='dataset')
    ...
    print("Building dataset with noaction swaps = "+str(swaps)+" and action random factor = "+str(rand_mul))
    random.seed(1)
    ...
    if swaps:
        # Create new no_action pairs
        new_no_action_list = []

        for item in no_action_list:
            class_1 = get_str_class(item[-2])
            class_2 = get_str_class(item[-1])

            random_img2 =random.choice(merged_class_dict[class_2])
            # the first image is the same as before, the second is randomly selected (to simulate random swaps)
            new_no_action_list.append((item[0], random_img2, item[2], class_1, class_2))
    else:
    ...
    #
    with open(pkl_dataset+"_ac_random_"+str(rand_mul)+"_noac_swaps_"+str(swaps)+".pkl", 'wb') as f:
